# MaizeLeafDisease-Kandahar-HybridData-MaxViT

This repository contains a hybrid dataset for maize (corn) leaf disease classification in Kandahar. It combines PlantVillage public data with field-collected data (with and without background). The dataset and experimental results are used to train, evaluate, and test the MaxViT deep learning model for automatic maize leaf disease detection.


## 📘 Maize Leaf Disease Model (MaxViT)
This repository contains the trained MaxViT model for maize leaf disease classification using actual-field and comdined data (with and without background).  
You can download or open the model notebook using the links below:

- [📥 Download MaxViT Model Notebook](https://github.com/RafiullahRafi/MaizeLeafDisease-Kandahar-HybridData-MaxViT/raw/main/MaxViT_model_on_with_and_without_background_data.ipynb)
- [🚀 Open in Google Colab](https://colab.research.google.com/github/RafiullahRafi/MaizeLeafDisease-Kandahar-HybridData-MaxViT/blob/main/MaxViT_model_on_with_and_without_background_data.ipynb)

[📦 Download MaxViT Notebook (ZIP)](https://github.com/RafiullahRafi/MaizeLeafDisease-Kandahar-HybridData-MaxViT/releases/download/v1.0/MaxViT_model.zip)
